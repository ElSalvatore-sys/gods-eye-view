...
test('every live basemap is reachable by its own id — no enum value without a voice alias', () => {
  ...
  // The tool schema moved to the shared route module so it is mounted in
  // production too (server/routes/realtime.js).
  const config = readFileSync(new URL('../../server/routes/realtime.js', import.meta.url), 'utf8');
