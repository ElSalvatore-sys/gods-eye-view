...
test('the voice TOOL SCHEMA is byte-identical to main — the mission mapping is instructions only', () => {
  const src = fs.readFileSync(new URL('./../server/routes/realtime.js', import.meta.url), 'utf8');
  ...
  // Re-pinned 2026-09-09: the layer-rainviewer mission DELIBERATELY extends
  // set_layer_visibility's enum with 'weather-radar' (a real new data layer —
  ...
  //
  // Re-pinned 2026-09-10: `manage_alerts` added — standing watches over the
  // existing src/alerts engine, which had no voice surface at all. Also a
  // deliberate schema growth, and it busts the session cache by design. What
