 ...
 *
 * Mounted at `/api/adsbdb` by both `vite.config.js` (dev + preview) and
 ...
 */
export function createAdsbdbRoute(deps = {}) {
  const TTL_MS = 24 * 3600_000;
  const fetchImpl = deps.fetchImpl || fetch;
  const CACHE_PATH = path.join(deps.cacheDir || path.join(process.cwd(), '.gev-cache'), 'adsbdb.json');
  ...
  return async function adsbdbRoute(req, res) {
    await loadOnce();
