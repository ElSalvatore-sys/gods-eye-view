 ...
 *
 * Mounted at `/api/tomtom` by both `vite.config.js` (dev + preview) and
 ...
 */
export function createTomtomRoute(deps = {}) {
  const TILE_TTL_MS = 120_000;
  const CACHE_DIR = deps.cacheDir || path.join(process.cwd(), '.gev-cache', 'tomtom');
  ...
  return async function tomtomRoute(req, res) {
