...
import { requiredFiniteQueryNumber } from '../lib/http.js';
 ...
 * Deliberately does not expose arbitrary Overpass QL to the browser — it
 * runs one fixed QL template through `fetchOverpassPayload` (shared with the
 * `/api/overpass` proxy, ./overpass.js) and rejects global, cross-dateline,
 * or oversized requests before touching public OSM mirrors. Moved verbatim
 * from `vite.config.js` (docs/ARCH-SERVER-SPLIT.md §4 PR 4).
 *
 * Mounted at `/api/military-installations` by both `vite.config.js` (dev +
 * preview) and `server/index.js` (prod) via the shared `server/routes.js`
 * manifest — unlike ./overpass.js's `/api/overpass`, no test resolves this
 * route by a dedicated Vite plugin name, so (matching the celestrak/PR1 and
 * launches/PR3 precedent) it installs solely through `gevRoutesPlugin()`; the
 * old `militaryInstallationsProxy()` plugin is removed from `vite.config.js`.
 *
 * @module server/routes/military
 */
 ...
 * truncated, so off-viewport features from the snapped bbox may have crowded
 * out in-viewport ones. Callers re-ask for the exact viewport in that case.
 */
...
/**
 * `/api/military-installations` handler.
 * @param {object} [deps] reserved for test overrides (none needed today —
 *   the persistence tests in `src/data/installationProxy.test.mjs` drive the
 *   exported disk/tier functions directly against a real temp directory).
 * @returns {(req: import('http').IncomingMessage, res: import('http').ServerResponse, next?: (err?: unknown) => void) => Promise<void>}
 */
export function createMilitaryInstallationsRoute(deps = {}) {
  ...
  return async function militaryInstallationsRoute(req, res) {
    if (req.method !== 'GET') {
