...
import { readRequestBodyCapped, readResponseTextCapped } from '../lib/http.js';
import { makeRateLimiter, clientKey } from '../lib/ratelimit.js';
import { haversineKm } from '../lib/upstream.js';

/**
 * Overpass API proxy (`/api/overpass`) and OSM routing proxy (`/api/route`).
 * Upstreams: 4 Overpass mirrors (`OVERPASS_UPSTREAMS`) and the FOSSGIS OSRM
 * servers (`routing.openstreetmap.de`). Moved verbatim from `vite.config.js`
 * (docs/ARCH-SERVER-SPLIT.md §4 PR 4) — the static QL validator
 * (`sanitizeOverpassBody`/`stripOverpassNoise`/`OVERPASS_*_RE`), the mirror
 * fan-out (`fetchOverpassPayload`), the rate limiters, and the
 * `.gev-cache/overpass/` disk-cache format are byte-for-byte unchanged: see
 * commits 973b4e1 ("rotate mirrors on any refusal, and never cache one as
 * data") and 7ffa421 ("reject legacy cached refusals") for why a 4xx must
 * rotate to the next mirror instead of ending the fan-out, and why a refusal
 * must never be written to (or read back from) the cache as if it were data.
 * `overpassPayloadIsData` is the one predicate both decisions go through so
 * they cannot drift apart again.
 *
 * Deliberately NOT migrated onto the `server/lib/cache.js` §5 contract in
 * this PR: `src/overpassProxy.test.mjs` (frozen — see the PR body) asserts
 * the exact `.gev-cache/overpass/<sha1>.json` disk path and payload shape
 * directly, and the module-level memory cache/in-flight maps below are that
 * same test's target when it drives the mounted `/api/overpass` handler.
 * Rewiring the disk layer through the shared cache would break it without
 * touching a file this PR is required to leave untouched.
 *
 * Mounted at `/api/overpass` and `/api/route` by both `vite.config.js` (dev)
 * and `server/index.js` (prod) via the shared `server/routes.js` manifest.
 * `vite.config.js` additionally keeps a thin plugin literally named
 * `overpass-proxy` wrapping `createOverpassRoute()` — real traffic is always
 * served by the manifest-mounted handler (registered first), but
 * `src/overpassProxy.test.mjs` resolves the plugin by that name directly, so
 * the name and its `configureServer` hook must keep existing.
 *
 * @module server/routes/overpass
 */
...
/**
 * `/api/overpass` handler: response caching + request coalescing over the
 * mirror fan-out above. Accepts POST requests, normalizes the query body for
 ...
 *
 * @param {object} [deps] reserved for test overrides (none needed today — the
 *   frozen `src/overpassProxy.test.mjs` drives this handler through the real
 *   module-level caches and mocks `globalThis.fetch` directly).
 * @returns {(req: import('http').IncomingMessage, res: import('http').ServerResponse, next?: (err?: unknown) => void) => Promise<void>}
 */
export function createOverpassRoute(deps = {}) {
  return async function overpassRoute(req, res) {
...
/**
 * `/api/route` handler: real OSM routing via the public FOSSGIS OSRM servers
 * (foot/car/bike). `GET /api/route?profile=foot|car|bike&coords=lon,lat;lon,lat[;...]`.
 * Every error is flattened to `{ok:false, error}` at HTTP 200 — a routing
 * miss is an ordinary outcome the client renders, not a proxy failure.
 *
 * @param {object} [deps] reserved for test overrides (none needed today).
 * @returns {(req: import('http').IncomingMessage, res: import('http').ServerResponse, next?: (err?: unknown) => void) => Promise<void>}
 */
export function createOsrmRoutingRoute(deps = {}) {
  return async function routeRoute(req, res) {
    const fail = (msg) => {
