 ...
 *
 * Mounted at `/api/terrain/heights` by both `vite.config.js` (dev + preview)
 ...
 */
export function createTerrainRoute(deps = {}) {
  const TTL_MS = 30 * 24 * 3600_000;
  const CACHE_DIR = deps.cacheDir || path.join(process.cwd(), '.gev-cache');
  ...
  return async function terrainRoute(req, res) {
