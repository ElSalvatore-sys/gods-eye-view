/**
 * Fixed-window rate limiting shared by every proxy route that needs a
 * backstop against a runaway or abusive client. Moved verbatim out of
 * `vite.config.js` (docs/ARCH-SERVER-SPLIT.md §4 PR 2) — no behaviour
 * changes, only the import path.
 *
 * @module server/lib/ratelimit
 */

/** Hard cap on distinct rate-limiter keys so a key-rotating caller can't grow the map without bound. */
 ...
 * @returns {(key: string) => boolean} `allow(key)`
 */
