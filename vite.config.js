...
import { launchLibraryRequestHeaders, LL2_CACHE_TTL_MS } from './server/routes/launches.js';
import {
  ...
  createOverpassRoute,
} from './server/routes/overpass.js';
import {
...
} from './server/routes/military.js';
// Re-exported below: src/overpassProxy.test.mjs, src/data/overpassProxy.test.mjs,
// src/data/installationProxy.test.mjs and src/data/regionalProxy.test.mjs still
// import these by name from this module (docs/ARCH-SERVER-SPLIT.md §4 PR 2, 3, 4).
export {
  readResponseTextCapped,
  readResponseJsonCapped,
...
// The Overpass API proxy (constants, `sanitizeOverpassBody`/`stripOverpassNoise`
// validator, disk cache, `fetchOverpassPayload` mirror fan-out) and OSM routing
// proxy moved to server/routes/overpass.js (docs/ARCH-SERVER-SPLIT.md §4 PR 4) —
// imported above and re-exported where a frozen test still imports by name.
...
/**
 * Mounts every route in the shared `server/routes.js` manifest onto both the
 * dev and preview middleware stacks, using the same `mount()` semantics
 * `server/index.js` uses in prod (docs/ARCH-SERVER-SPLIT.md §2a, §3) — so
 * dev, preview, and prod run the identical handler objects. Currently
 * carries only CelesTrak (PR 1 of the server split); routes not yet moved
 * keep registering their own plugin below, unchanged. Registering
 * `configurePreviewServer` fixes the §1b preview-parity gap: these routes
 * used to fall through to the SPA index under `vite preview`.
 * @returns {import('vite').Plugin}
 */
function gevRoutesPlugin() {
  const install = (server) => {
    for (const { mount: routeMount, handler } of ROUTES) {
      server.middlewares.use(routeMount, handler);
    }
  };
  return {
    name: 'gev-routes',
...
/**
 * `/api/overpass`, `/api/route`, and `/api/military-installations` were moved
 * to server/routes/{overpass,military}.js (docs/ARCH-SERVER-SPLIT.md §4 PR 4)
 * and now install via `gevRoutesPlugin()` above, same as celestrak (PR 1) and
 * the PR 3 routes — that registration runs first in the `plugins` array below
 * and serves every real dev/preview/prod request.
 *
 * This plugin is a compatibility shim wrapping the SAME handler
 * (`createOverpassRoute()`) purely so `src/overpassProxy.test.mjs` — frozen by
 * docs/ARCH-SERVER-SPLIT.md §4 PR 4 — can still resolve a plugin literally
 * named `overpass-proxy` and drive `/api/overpass` through it directly. Its
 * `configureServer` registration is never reached by a live server (the
 * `gev-routes` mount above already ended the response), so this function has
 * no runtime effect outside that test.
 * @returns {import('vite').Plugin}
 */
function overpassProxy() {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/overpass', createOverpassRoute());
    },
  };
}
 ...
 */
// openAiRealtimeProxy REMOVED 2026-09-10. Its two routes — /api/realtime/token
// and /api/realtime/debug-log — moved to server/routes/realtime.js and are now
// mounted from the shared server/routes.js manifest, so they exist in
// production as well as dev. Living here made them dev-only: `node
// server/index.js` answered /api/realtime/token with sirv's SPA fallback
// (index.html, text/html), so the voice agent was silently dead in every
// deployed build.

// readRequestBody moved to server/lib/http.js (docs/ARCH-SERVER-SPLIT.md §4 PR 2) — imported above.
// extractOpenAiResponseText / toFiveWordHudSummary moved to server/lib/aiProvider.js and
// server/routes/hudSummary.js respectively, alongside the hud-summary route they served.
...
// The military-installation context proxy moved to server/routes/military.js
// (docs/ARCH-SERVER-SPLIT.md §4 PR 4) — imported above and re-exported where a
// frozen test still imports by name.
...
// requiredFiniteQueryNumber moved to server/lib/http.js (docs/ARCH-SERVER-SPLIT.md
// §4 PR 4) — imported above; still used here by validRegionalPoint below and by
